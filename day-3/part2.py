...
def extract_number_near_position(
...
def get_gear_ration_near_symbol(
    ...
    for x in range(-1, 2):
        for y in range(-1, 2):
            (characters_grid, numbers) = extract_number_near_position(characters_grid, symbol_x + x, symbol_y + y)
            all_numbers = [*all_numbers, *numbers]
    if len(all_numbers) != 2:
        return characters_grid, 0
    return characters_grid, all_numbers[0] * all_numbers[1]
...
def extract_numbers_near_symbols(
    ...
    for symbol in symbols:
        (characters_grid, gear_ration) = get_gear_ration_near_symbol(symbol, characters_grid)
        all_numbers = [*all_numbers, gear_ration]
...
def main():
    ...
    (characters, numbers) = extract_numbers_near_symbols(symbol_locations, characters)
